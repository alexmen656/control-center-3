...
const filteredProjectFiles = computed(() => {
  if (exclude.value) {
    ...
    // Zeige alle Dateien an, aber excludiere nur bestimmte Monaco-interne Dateien aus der Explorer-Anzeige
    // .monaco_apis/ Dateien sollen im Explorer sichtbar sein, aber NICHT in Git Changes
...
const loadGitData = async () => {
  try {
    ...
    if (changesResponse.data.success) {
      // Filter out ONLY excluded files from changes display, but KEEP .monaco_apis files visible
      const allChanges = changesResponse.data.changes || []
      changedFiles.value = allChanges.filter(file => {
        const filePath = file.path || file.file || ''
        // Only exclude Monaco-internal files, but KEEP .monaco_apis files visible in changes
        return !excludedFiles.value.includes(filePath)
      })
    } else {
      changedFiles.value = []
    }
...
const commitChanges = async () => {
  ...
  try {
    ...
    // Get ALL changes including .monaco_apis files for commit (not just the filtered ones displayed)
    const allChangesResponse = await axios.get(`monaco_git_api.php?project=${projectName}&action=changes`)
    let allChangedFiles = []
    
    if (allChangesResponse.data.success) {
      allChangedFiles = allChangesResponse.data.changes || []
    }

    // Prepare files for commit - include ALL changed files
    const filesToCommit = allChangedFiles.map(f => ({
      path: f.path || f.file,
      status: f.status,
      staged: f.staged
    }))
