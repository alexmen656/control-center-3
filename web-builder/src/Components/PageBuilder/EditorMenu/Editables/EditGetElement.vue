...
// Holen des ausgewählten Text-Elements
const getSelectedTextElement = computed(() => {
  return pageBuilderStateStore.getSelectedTextElement;
});

// Fix: Direkte Überwachung des TipTap-Modal-Status ohne Rekursion
const getShowModalTipTap = computed(() => {
  return pageBuilderStateStore.getShowModalTipTap;
});

// Überwache Änderungen am Modal-Status und reagiere entsprechend
...
// Neue Hilfsfunktion zum Einrichten des Modals
const setupModalForTipTap = function () {
  typeModal.value = 'success';
  gridColumnModal.value = 2;
  titleModal.value = getSelectedTextElement.value ? 'Text-Element bearbeiten' : 'Manage Content';
...
// Korrigierte Funktion ohne rekursiven Aufruf
const handleModalPreviewTiptap = function () {
  pageBuilderStateStore.setShowModalTipTap(true);
  // Modal-Einrichtung wird jetzt durch den Watcher übernommen
};
