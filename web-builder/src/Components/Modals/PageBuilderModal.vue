...
const saveOnly = async function () {
  try {
    ...
    const response = await fetch(
      `https://api.fringelo.com/web-builder/publish_vue.php?project_id=${currentProject.value.id}&deploy=true`,
      {
        method: 'GET',
        headers: {
...
<template>
  <teleport to="body">
    <TransitionRoot :show="show" as="template">
      <Dialog @close="firstButton" @keydown.escape.prevent="handleEscapeKey" as="div"
        class="fixed z-30 inset-0 overflow-y-auto focus:outline-none" tabindex="0">
        <div class="flex items-end justify-center pb-20 text-center sm:block sm:p-0 bg-white">
          ...
          <TransitionChild as="template" enter="ease-out duration-100"
            ...
            <div
              ...
              <DynamicModal :show="showModalConfirmClosePageBuilder" :type="typeModal"
                :gridColumnAmount="gridColumnModal" :title="titleModal" :description="descriptionModal"
                :firstButtonText="firstButtonModal" :secondButtonText="secondButtonModal"
              ...
              <div @click="pageBuilderStateStore.setComponent(null)"
                ...
                <div class="flex items-center justify-start"><!-- divide-x divide-gray-200-->
                  <button type="button" @click="firstButton" class="pr-6"><!--border-r border-gray-200 -->
                    <img class="h-6" src="/logo/logo.png" alt="Logo" />
                  </button>
                </div>
                <button class="myPrimaryButton btn-red lg:text-sm text-[10px] lg:py-2 py-2 min-h-2 ml-4"
                  @click="saveOnly" type="button">
