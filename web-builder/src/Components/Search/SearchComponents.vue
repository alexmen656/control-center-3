...
<template>
  <Modal maxWidth="5xl" :show="show" @close="firstButton" minHeight="" maxHeight="">
    <div
      ...
      <div class="p-4">
        ...
        <template v-if="getFetchedComponents && getFetchedComponents.isLoading">
          <SmallUniversalSpinner class="h-40" width="w-6" height="h-6" border="border-4"></SmallUniversalSpinner>
        </template>
        <template v-if="
          !getFetchedComponents.isError && !getFetchedComponents.isLoading
        ">
          <div class="flex gap-2 flex-wrap">
            <button @click="handlecategorySelected({ name: 'Components', id: null })" class="myPrimaryTag font-medium"
          ...
          <div class="h-full flex md:flex-row flex-col myPrimaryGap mt-2 p-2 overflow-y-scroll">
                ...
                <div
                  ...
                  <div
                    ...
                    v-for="component in getFetchedComponents &&
                      ...
                      getFetchedComponents.fetchedData.components.data" :key="component.id">
                    <div class="relative">
                      <template v-if="component && component.cover_image">
                        <img :src="component.cover_image" alt="placeholder"
                  ...
                  <div class="flex gap-4 flex-wrap w-full">
                    <div v-for="helperComponent in componentHelpers" :key="helperComponent.title">
                      <div class="flex justify-left items-center gap-4 text-xs font-medium">
                        <button @click="handleAddHelperComponent(helperComponent)" type="button"
            ...
            <aside class="md:w-2/6 overflow-scroll min-h-[30rem] max-h-[30rem] w-full border rounded-lg py-4 px-2">
              <div class="flex gap-4 flex-wrap w-full">
                <div v-for="helperComponent in componentHelpers" :key="helperComponent.title">
                  <div class="flex justify-left items-center gap-4 text-xs font-medium">
                    <button @click="handleAddHelperComponent(helperComponent)" type="button" class="mySecondaryButton">
                      <span class="material-symbols-outlined text-sm">
                        add
                      </span>
                      <span>
    ...
    <div v-if="!getFetchedComponents.isError" class="bg-slate-50 px-2 py-4 flex sm:justify-end justify-center">
      <div class="sm:w-3/6 w-full px-2 my-2 flex gap-2 justify-end">
        <button v-if="firstButtonText" ref="firstButtonRef" class="mySecondaryButton" type="button"
          @click="firstButton">
          {{ firstButtonText }}
        </button>
      </div>
