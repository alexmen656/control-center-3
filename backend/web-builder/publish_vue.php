...
<body class="antialiased">
    ...
    const ContentParser = {
        ...
        parseLoops(html, data) {
            const startRegex = /\{%\s*for\s+([a-zA-Z_][a-zA-Z0-9_]*)\s+in\s+([a-zA-Z_][a-zA-Z0-9_]*(?:\.[a-zA-Z0-9_]+)?)((?:\s*\|\s*[a-zA-Z0-9_=:\"'\-]+)*)\s*%\}/;
            ...
            let processed = '';
            let tableData = this.resolveData(data, tableName);

            if (tableData && Array.isArray(tableData)) {
                tableData = this.applyLoopModifiers([...tableData], modifiersStr);

                // Export pagination data to the current scope if available
                if (tableData.pagination) {
                    data.pagination = tableData.pagination;
                }

                processed = tableData.map(row => {
                    let item = loopBody;
                    
                    // Allow nested loops by passing current row as context
                    const localData = { ...data, [loopVar]: row };
                    item = this.parseLoops(item, localData);
                    ...
                    // Primitive/Direct Loop Variable: {{ loopVar }}
                    const simpleVarPattern = new RegExp(
                        '\\\\{\\\\{\\\\s*' + this.escapeRegex(loopVar) + '((?:\\\\s*\\\\|\\\\s*.*?)?)\\\\s*\\\\}\\\\}',
                        'g'
                    );

                    item = item.replace(simpleVarPattern, (m, mods) => {
                        // If direct access match was already handled by dot-notation (unlikely due to regex diff), skip
                        // But since dot notation regex is specific, this one should match {{ loopVar }} only if no dot unless we are careful.
                        // Actually regex 'loopVar' followed by modifiers/end brace will NOT match 'loopVar.something'.
                        // So we are safe.
                        
                        let val = row;
                        // But if row is object, stringifying it might be [object Object]
                        if (typeof row === 'object' && row !== null) {
                            // If user tries {{ obj }} probably wants json or nothing? 
                            // For backward compat or safety we might ignore or empty?
                            // But for pagination (numbers) this is exactly what we want.
                        }
        ...
        parseLoopConditions(html, loopVar, row) {
            ...
            html = html.replace(ifPattern, (match, condition) => {
                // 1. Resolve loopVar.property
                const varRefPattern = new RegExp(
                    this.escapeRegex(loopVar) + '\\\\s*\\\\.\\\\s*([a-zA-Z_][a-zA-Z0-9_]*)',
                    'g'
                );

                let resolvedCondition = condition.replace(varRefPattern, (m, column) => {
      ...
      const simpleVarPattern = new RegExp(
                ...
                resolvedCondition = resolvedCondition.replace(simpleVarPattern, (m) => {
                     let value = row;
                     if (typeof row === 'object' && row !== null) return m; // Avoid object replacement
        ...
        evaluateCondition(condition, data) {
            ...
            let resolved = condition.replace(
                ...
                (match, table, column, index) => {
                    ...
                    const escaped = String(value !== undefined && value !== null ? value : '').replace(/\\\\/g, '\\\\\\\\').replace(/"/g, '\\\\\\"');
                    return '"' + escaped + '"';
                }
            );

            // Resolve general variable paths (e.g. pagination.current_page)
            resolved = resolved.replace(
                /([a-zA-Z_][a-zA-Z0-9_]*)\s*\.\s*([a-zA-Z_][a-zA-Z0-9_]*)(?!\w)(?![\[])/g,
                (match, objName, propName) => {
                    const obj = data[objName];
                    if (!obj) {
                        // console.warn('Object not found:', objName);
                        return '""';
                    }
                    ...
                    // console.log('Resolved property:', objName, propName, value);
                    const escaped = String(value !== undefined && value !== null ? value : '').replace(/\\\\/g, '\\\\\\\\').replace(/"/g, '\\\\\\"');
                    return '"' + escaped + '"';
                }
            );

            console.log('Final Resolved Condition:', resolved);

            // Equality check: "a" == "b"

            // Equality check: "a" == "b"
            const eqMatch = resolved.match(/^"([^"]*)"\s*==\s*"([^"]*)"$/);
            if (eqMatch) {console.log(eqMatch[1] === eqMatch[2]);};

            if (eqMatch) return eqMatch[1] === eqMatch[2];

            // Fallback: Number equality check (if quoting failed)
            const numEqMatch = resolved.match(/^(\d+)\s*==\s*(\d+)$/);
            if (numEqMatch) {console.log(parseInt(numEqMatch[1]) === parseInt(numEqMatch[2]));};
            if (numEqMatch) return parseInt(numEqMatch[1]) === parseInt(numEqMatch[2]);

            // Robust Fallback: Generic Equality (strips quotes and compares loosely)
            const splitEq = resolved.split('==');
            if (splitEq.length === 2) {
                const left = splitEq[0].trim().replace(/^"|"$/g, '');
                const right = splitEq[1].trim().replace(/^"|"$/g, '');
                // Allow type coercion (1 == "1")
                return left == right;
            }
            ...
            // Tconst modLower = mod.toLowerCase();
        ...
        applyLoopModifiers(data, modifiersStr) {
            ...
            modifiers.forEach(mod => {
                ...
                // Slice: slice:start:end
                    ...
                    const end = parts[2] ? parseInt(parts[2]) : undefined;
                    data = data.slice(start, end);
                }
                // Pagination: paginate:perPage
                else if (mod.startsWith('paginate:')) {
                    const perPage = parseInt(mod.split(':')[1]) || 10;
                    let currentPage = 1;
                    ...
                    paginatedData.pagination = {
                        ...
                        start_index: totalItems === 0 ? 0 : Math.min(start + 1, totalItems),
                        end_index: Math.min(end, totalItems),
                ...
                // Search: search:query
                else if (mod.startsWith('search:')) {
                    const query = mod.split(':')[1];
                ...
                // Group By: group_by:key
                else if (mod.startsWith('group_by:')) {
                     const key = mod.substring(9).trim().replace(/['"]/g, '');
                     const groups = {};
                     ...
                     data.forEach(row => {
                         ...
                         if (!groups[groupVal]) {
                             groups[groupVal] = { name: groupVal, items: [] };
                         }
                         groups[groupVal].items.push(row);
                     });
                     
                     data = Object.values(groups);
                }
                // Reverse
                else if (modLower === 'reverse') {
                    data.reverse();
                }
            });
            return data;
