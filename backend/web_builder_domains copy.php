...
/**
 * SAVE - Speichern einer neuen Subdomain oder Custom Domain
 */
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'save') {
    ...
    $custom_base_domain = escape_string($_POST['custom_base_domain'] ?? '');
    ...
    if ($domain_type === 'custom' && $isSuperAdmin) {
        ...
        if (!$custom_base_domain) {
            echo json_encode(['success' => false, 'error' => 'Custom Base Domain ist erforderlich']);
            exit;
        }
        ...
        if ($subdomain && strlen($subdomain) > 0) {
            ...
            $fullDomain = $subdomain . '.' . $custom_base_domain;
        } else {
            // Ohne Subdomain: customdomain.com (root domain)
            $fullDomain = $custom_base_domain;
        }
    } else {
        // Regular Subdomain
        if (!$main_domain) {
            echo json_encode(['success' => false, 'error' => 'Main Domain ist erforderlich']);
            exit;
        }
        
        if (!$subdomain) {
            echo json_encode(['success' => false, 'error' => 'Subdomain ist erforderlich']);
            exit;
        }
    ...
    // Prüfen ob Domain bereits existiert (nur bei neuen Einträgen)
    $checkExisting = query("SELECT id, projectID FROM web_builder_domains WHERE domain='$fullDomain'");
    if ($existingRow = fetch_assoc($checkExisting)) {
        if ($existingRow['projectID'] !== $project) {
            echo json_encode(['success' => false, 'error' => 'Diese Domain ist bereits vergeben']);
            exit;
        }
    }

    // Prüfen ob für dieses Projekt bereits eine Domain existiert
    $checkProject = query("SELECT id, cloudflare_record_id, domain FROM web_builder_domains WHERE projectID='$project' LIMIT 1");

    if ($existingProject = fetch_assoc($checkProject)) {
        // Update existierende Domain

        // Alten Cloudflare Record löschen falls vorhanden
        $oldDomain = $existingProject['domain'];
        if (!empty($existingProject['cloudflare_record_id'])) {
            // Lösche mit Zone-Erkennung basierend auf alter Domain
            cloudflare_deleteRecordByDomain($oldDomain);
        }
        ...
        $updateSql = "UPDATE web_builder_domains 
                      SET subdomain='$subdomain', 
                          domain='$fullDomain',
                          main_domain=" . ($main_domain ? "'$main_domain'" : "''") . ",
        ...
        if (query($updateSql)) {
            ...
            echo json_encode([
                'success' => true,
                'message' => 'Domain und DNS erfolgreich konfiguriert. SSL-Zertifikat wird automatisch erstellt.',
                'domain' => $fullDomain,
        ...
        $insertSql = "INSERT INTO web_builder_domains (projectID, subdomain, domain, main_domain, is_enabled, cloudflare_record_id, ssl_status) 
                      VALUES ('$project', '$subdomain', '$fullDomain', " . ($main_domain ? "'$main_domain'" : "''") . ", " . ($is_enabled ? '1' : '0') . ", " . ($cloudflareRecordId ? "'$cloudflareRecordId'" : "NULL") . ", 'pending')";
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'delete') {
    ...
    $getDomainSql = "SELECT cloudflare_record_id, domain FROM web_builder_domains WHERE projectID='$project' LIMIT 1";
    $domainResult = query($getDomainSql);

    if ($domainRow = fetch_assoc($domainResult)) {
        $domain = $domainRow['domain'];
        if (!empty($domain)) {
            // Lösche mit automatischer Zone-Erkennung
    ...
    if (query($deleteSql)) {
        echo json_encode([
            'success' => true,
            'message' => 'Domain und DNS erfolgreich gelöscht'
        ]);
    } else {
